...
const SystemDesign: React.FC<SystemDesignProps> = ({ theme }) => {
...
`;
  ...
  return (
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      {/* Migration Alert Box */}
      <div className={`border p-8 rounded-[2.5rem] shadow-2xl relative overflow-hidden group transition-colors ${isGenie ? 'bg-purple-500/5 border-purple-500/20' : 'bg-cyan-500/5 border-cyan-500/20'}`}>
        <div className="absolute top-0 right-0 p-8 opacity-5 group-hover:opacity-10 transition-opacity">
          <Zap size={120} className={isGenie ? "text-purple-400" : "text-cyan-400"} />
        </div>
        
        <div className="flex flex-col md:flex-row items-center justify-between gap-8 relative z-10">
          <div className="flex items-start gap-5">
            <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 ${isGenie ? 'bg-purple-500/20' : 'bg-cyan-500/20'}`}>
              <AlertTriangle className={isGenie ? "text-purple-400" : "text-cyan-400"} size={28} />
            </div>
            <div>
          ...
          <button 
            ...
            className={`w-full md:w-auto text-white px-8 py-4 rounded-2xl font-black text-sm uppercase tracking-widest transition-all active:scale-95 shadow-lg whitespace-nowrap ${
              isGenie 
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className={`bg-slate-900/40 border border-slate-800 p-8 rounded-3xl space-y-4 transition-colors ${isGenie ? 'hover:border-purple-500/30' : 'hover:border-cyan-500/30'}`}>
          <Globe className={isGenie ? "text-purple-400" : "text-cyan-400"} size={32} />
        ...
        <div className={`bg-slate-900/40 border border-slate-800 p-8 rounded-3xl space-y-4 transition-colors ${isGenie ? 'hover:border-pink-500/30' : 'hover:border-emerald-500/30'}`}>
          <Cpu className={isGenie ? "text-pink-400" : "text-emerald-400"} size={32} />
        ...
        <div className={`bg-slate-900/40 border border-slate-800 p-8 rounded-3xl space-y-4 transition-colors ${isGenie ? 'hover:border-indigo-500/30' : 'hover:border-purple-500/30'}`}>
          <Shield className={isGenie ? "text-indigo-400" : "text-purple-400"} size={32} />
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between px-2">
          <div className={`flex items-center gap-3 ${isGenie ? 'text-pink-400' : 'text-emerald-400'}`}>
            <Lock size={24} />
            <h3 className="text-2xl font-bold text-white tracking-tight">Full Schema Script</h3>
          </div>
          <button 
            onClick={() => handleCopy(fullSchemaSql)}
            className={`text-[10px] font-black uppercase tracking-[0.2em] text-slate-500 transition-colors ${isGenie ? 'hover:text-pink-400' : 'hover:text-emerald-400'}`}
          >
            Copy Full SQL
          </button>
        </div>
        
        <div className={`bg-slate-950 p-8 rounded-[2.5rem] border border-slate-800 font-mono text-[11px] overflow-x-auto max-h-[400px] overflow-y-auto leading-relaxed scrollbar-thin shadow-2xl ${isGenie ? 'text-pink-500/60' : 'text-emerald-500/60'}`}>
